...
## 8. PR #6 review record (post-creation, same branch)

A pull request (PR #6) was opened from this branch after Section 7's implementation record. Two automated reviewers left findings on the diff; both rounds followed the same discipline as Section 7 — every finding was empirically reproduced against the pre-fix code (a throwaway or permanent regression test confirmed to fail first) before the fix was applied, then confirmed to pass after.

**Gemini Code Assist round (6 findings, all fixed, committed as `6af0024`/`8706473`/`8d764fa`):** an unbounded `limit_lines` overflow in `chattools.go`'s `readFile` (Go int addition wraps rather than panicking; the actual danger zone is `math.MaxInt64-2047`, not `math.MaxInt64` itself, because of float64's precision gap near 2^63 combined with JSON round-tripping — see the commit for the full mechanism), a path-containment check tightened from a bare `strings.HasPrefix` to a `filepath.Rel`-based check (rejecting `../` escapes without also rejecting a legitimately-named `..foo` path), `isChatSecretPath` switched to `path.Base`/`path.Match` for platform-independent matching, and `searchFiles`' line-truncation switched from a `[]rune` allocation to byte-index walking. Also: a flaky CI test (`TestRunsAPIStartErrorsAreHumanized`) traced to a background engine goroutine racing `t.TempDir()`'s cleanup after a successful Start — fixed by polling for a terminal run status before returning, mirroring the pattern the suite's other full-lifecycle test already used.

**Codex automated review round (9 findings, all fixed, committed as `028ea14`/`9bcc12e`/`3a2c50e`/`5f31694`/`40a9457`/`77f5ee5`):**

1. **Ledger-append race reintroducing the original Bug 2 symptom** (`engine.go`) — the most severe finding. `StartRun` wrote the auto-checkpoint's `ledger.md` entry immediately after `AutoCheckpoint`'s commit but before `EnsureRunBranch`'s checkout; if `ledger.md` is already git-tracked (any run after the first, or a Planning-Mode-scaffolded repo), that write re-dirties a tracked file right before gogit's whole-tree dirty check — reproducing "worktree contains unstaged changes" with `ledger.md` as the trigger instead of `lock.json`. Fixed by moving the `AppendLedger` call to after the checkout succeeds. `TestLedgerAppendBeforeCheckoutReDirtiesTrackedFile`/`TestLedgerAppendAfterCheckoutSucceeds` added.
2. **Untracked-directory collapse hides secrets from AutoCheckpoint's gate** (`gitx/exec.go`) — real git's default `status --porcelain` mode collapses an entirely-untracked directory to one `?? dirname/` line, so a credential file inside a brand-new directory never matched `AutoCheckpoint`'s per-path Denylist/secret checks under the exec backend, while `git add -A` staged and committed it anyway. Fixed by adding `--untracked-files=all`; gogit is unaffected by construction. `TestStatusPorcelainListsFilesInsideUntrackedDirectories` added (both backends).
3. **Repo registration ignored an active foreign lock** (`gateway/repos.go`) — `repoRegisteredResponse` scaffolded `.l00prite/` unconditionally, even while another agent held an active, unexpired lock — racing that agent's in-progress understanding of the folder, against LOCKING.md's own convention. Fixed by checking `LockAvailability` first and skipping scaffolding (with an explanatory note) on a foreign lock. `TestRepoRegisteredResponseRespectsForeignLock` added.
4. **`read_file` loaded the whole file before truncating** (`gateway/chattools.go`) — switched from `os.ReadFile` to `os.Open` + `io.LimitReader(cap+1)`, mirroring the engine's own governed `Toolbox.readFile`. `TestChatToolboxReadFileTruncatesLargeFileWithoutLoadingItWhole` added.
5. **`search_files` had no bound on total files walked** (`gateway/chattools.go`) — a rare or non-matching query against a very large repo walked and opened every eligible file with no cap. Added `chatSearchMaxFilesWalked` (a `var`, not a `const`, so tests can lower it instead of creating tens of thousands of fixture files) plus a truncation-aware "no matches" message. `TestChatToolboxSearchFilesBoundsFilesWalked` added.
6. **Round-cap exhaustion could return dangling `tool_calls` to the client** (`gateway/chatloop.go`) — `forcedFinal` only tripped on the per-turn tool-call-count cap, not the round cap itself; a model calling tools right up to the last round got its calls executed locally with the results never sent back for a real answer, leaving the returned response with unresolved `tool_calls` the client doesn't own. Fixed by also forcing the last round (`round == chatMaxToolRounds-1`) final.
7. **Stale `tool_choice` could reference a tool `stripChatTools` just removed** (`gateway/chatloop.go`) — mirrored `stripBridgeTool`'s existing `tool_choice`-clearing pattern. `TestStripChatToolsClearsToolChoiceReferencingRemovedTool` added.
8. **A failed commit left the index staged** (`engine/tools.go`) — `CommitUnit`'s `AddAll` stages everything before `Commit` runs; a `Commit` failure for any reason other than "nothing to commit" or a missing identity (a rejecting hook, a full disk) left that staging in place with no commit to show for it. Fixed with a best-effort `git reset` via the exec backend's `Raw()` passthrough (gogit's `Raw` is unconditionally unsupported — closing that gap fully would need a new `gitx.Client` interface method across both backends and every test double, judged disproportionate to this finding's severity and documented as a known asymmetry rather than left silent). `TestCommitUnitResetsIndexOnFailedCommit` added.

...
3. The native Activity exchanges the install secret at `POST /v1/setup/session`, installs the
...
| G7 | Any co-installed app can reach `127.0.0.1:8787`; the pre-latch setup window needs its own authentication | `LOOPRITE_SETUP_SECRET` is accepted only by a native one-time exchange at `/v1/setup/session`. The resulting HttpOnly, SameSite=Strict cookie gates setup mutations. The secret never enters a URL or JavaScript; replay while a session is active is rejected. Desktop behavior remains unchanged when the env is unset. | `gateway/setup_session.go`, `gateway/setup.go`, wrapper |
